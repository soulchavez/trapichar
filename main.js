...
(function () {
  ...
  function createArrowMarkerContent(bearingDeg) {
    ...
    glow.style.background =
      "radial-gradient(circle, rgba(10,122,251,0.3) 0%, transparent 70%)";
  ...
  function clusterPoints(validPoints, zoom) {
    ...
    for (let i = 0; i < validPoints.length; i++) {
      ...
      const cluster = {
        ...
        centerLat: validPoints[i].lat,
        centerLng: validPoints[i].lng,
      };
      assigned[i] = true;

      const pixelI = latLngToPixel(
        validPoints[i].lat,
        validPoints[i].lng,
        zoom,
      );
      ...
      try {
        ...
        if (cluster.points.length > 1) {
          ...
          const clickHandler = () => {
            ...
            cluster.points.forEach((p) =>
              clusterBounds.extend({ lat: p.lat, lng: p.lng }),
            );
          ...
          if (isAdvanced) {
            markerOptions.content = createSingleMarkerContent(
              firstPunto.ubicacion,
            );
          } else {
            markerOptions.icon = {
              url: "./assets/icons/store.svg",
...
function traceRouteToPoint(dest) {
  ...
  function openCompanyModal(
    message = "Elige un producto y encuentra donde comprar",
  ) {
